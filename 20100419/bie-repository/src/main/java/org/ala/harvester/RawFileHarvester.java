...
public class RawFileHarvester implements Harvester {
	...
	private final String TARGET_FILE_NAME = FileType.RAW.getFilename()+".html";
...
//					System.out.println(rawFile.getAbsolutePath());
	...
	private List<File> getRawFileList(File infoSourceDir) {
		...
		if (infoSourceDir.isDirectory()) {
			logger.info("Start crawling " + TARGET_FILE_NAME);
			searchForAllFileByName(rawFileList, infoSourceDir, TARGET_FILE_NAME);
		}

		return rawFileList;
	}

	private void searchForAllFileByName(List<File> resultFileList, File rootDir, String targetFileName) {
		...
		for (File file : files) {
			...
			if (file.getName().matches(targetFileName)) {
				...
				searchForAllFileByName(resultFileList, file, targetFileName);
			}
		}
	}
	...
	private void harvestDoc(int infosourceId, File rawFile) throws Exception {
		...
		String url = rawFile.getAbsolutePath();
