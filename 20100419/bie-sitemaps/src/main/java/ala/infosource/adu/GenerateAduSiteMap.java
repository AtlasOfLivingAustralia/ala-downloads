...
public class GenerateAduSiteMap {
	...
	public static void main(String[] args) throws Exception{
		...
		Pattern genusIndexPattern = Pattern.compile(
				"(?:<li><span class='FontMedium'><a href=)?" +
				"(biota_details\\.aspx\\?BiotaID=[0-9]{1,})" + //link to genus page (1)
				"(?:>)?" +
				"(["+ExtractUtils.SCIENTIFIC_NAME_PATTERN_SIMPLE+"]{1,})" + //genus name (2)
				"(?:</a></span><span class='FontSmall'>[ ]*\\-[ ]*Subfamily[ ]*<a href=)?" +
				"(biota_details\\.aspx\\?BiotaID=[0-9]{1,})" + //link to subfamily page (3)
				"(?:>)?" +
				"(["+ExtractUtils.SCIENTIFIC_NAME_PATTERN_SIMPLE+"]{1,})"+	//subfamily name (4)
				"(?:</a>)?" +
				"(?:.*)?" +
				"(child_list\\.aspx\\?BiotaID=[0-9]{1,})"  //link to species page (5)
				);
		
		Pattern speciesIndexPattern= Pattern.compile(
			"(?:<a href=)?" +
			"(biota_details\\.aspx\\?BiotaID=[0-9]{1,})" +
			"(?:>)?" +
		...
		ExtractUtils.writeColumnHeaders(writer, new String[]{ColumnHeaders.URI, ColumnHeaders.SCIENTIFIC_NAME, 
				ColumnHeaders.TAXON_RANK, ColumnHeaders.SUBFAMILY, ColumnHeaders.GENUS, 
				ColumnHeaders.SPECIFIC_EPITHET});
		...
		while(m.find(searchIdx)){
			...
			while(m2.find(searchIdx2)){
				...
				//write out species page URL info
	...
	private static void writeOutInfo(Writer writer, String pageUrl,
			String scientificName, String taxonRank, String subfamilyName, String genusName, String specificEpithet) throws IOException {
