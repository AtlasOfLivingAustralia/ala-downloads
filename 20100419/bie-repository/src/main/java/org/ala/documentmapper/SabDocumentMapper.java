...
public class SabDocumentMapper implements DocumentMapper{
	...
	public List<ParsedDocument> map(String uri, byte[] content) throws Exception {
		...
		String sciName = null;
		String hasConservationStatus = null;
		System.out.println(documentStr);

		String[] doc = documentStr.split("[\\s&&[^ ]]{2,}");

		for (int i = 0; i < doc.length; i++) {
//			System.out.println("STR!!!" + i + "!!!" + doc[i]);

			if (i == 0) {
				if (doc[i].contains("Threatened Flora")){

					hasConservationStatus = doc[i].replaceAll("Threatened Flora on Kangaroo Island", "").trim();
		...
		triples.add(new Triple(subject, Predicates.CONSERVATION_STATUS.toString(), hasConservationStatus));
